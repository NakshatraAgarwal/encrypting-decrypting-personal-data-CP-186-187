...
def saveData():
    ...
    cipher_hex = ciphercode.hex()
    print(cipher_hex)
    file.write(cipher_hex)
...
def viewData():
    ...
    text_file = filedialog.askopenfilename(title="Select File",filetype = (("Text Files", "*.txt"),))
    name = os.path.basename(text_file)
    print(name)
    text_file = open(name,"r")
    para = text_file.read()
    byte_str = bytes.fromhex(para)
